...
func NewClientCoreData() *ClientCoreData {
	...
	earlyFlags := uint16(RNS_UD_CS_SUPPORT_ERRINFO_PDU)
	if experimentalRDPGFXEnabled() {
		earlyFlags |= RNS_UD_CS_SUPPORT_DYNVC_GFX_PROTOCOL
	}
...
func NewClientNetworkData() *ClientNetworkData {
	n := &ClientNetworkData{}
	n.ChannelDefArray = make([]ChannelDef, 0, 4)
	...
	if experimentalRDPGFXEnabled() {
		var dyn ChannelDef
		dyn.Name = plugin.DRDYNVC_SVC_CHANNEL_NAME
		dyn.Options = uint32(CHANNEL_OPTION_INITIALIZED | CHANNEL_OPTION_ENCRYPT_RDP |
			CHANNEL_OPTION_COMPRESS_RDP | CHANNEL_OPTION_SHOW_PROTOCOL)
		n.ChannelDefArray = append(n.ChannelDefArray, dyn)
	}
	...
	n.ChannelCount = uint32(len(n.ChannelDefArray))
