...
type SettingsPanel = 'appearance'|'updates'|'securitySync'|'knownHosts'|'transfer';
...
const isTerminalPasteShortcut = (e: globalThis.KeyboardEvent) => (e.ctrlKey || e.metaKey) && !e.altKey && !e.shiftKey && e.key.toLowerCase() === 'v';
...
function App() {
  ...
  function attachTerm(id: string, el: HTMLDivElement | null) {
    ...
    if (existing) {
      ...
      if (existing.keyDownHandler) existing.el.removeEventListener('keydown', existing.keyDownHandler, {capture:true});
    ...
    const keyDownHandler = (e: globalThis.KeyboardEvent) => { if (isTerminalPasteShortcut(e)) { e.preventDefault(); e.stopPropagation(); void pasteTerminalClipboard(id); } };
    el.addEventListener('contextmenu', contextMenuHandler, {capture:true});
    el.addEventListener('paste', pasteHandler, {capture:true});
    el.addEventListener('keydown', keyDownHandler, {capture:true});
    const rec: TerminalRecord = { term, fit, el, expectedSeq: 1, seqBuffer: {}, contextMenuHandler, pasteHandler, keyDownHandler };
  ...
  async function closeTerminal(id: string, e?: MouseEvent) {
    ...
    if (rec?.keyDownHandler) rec.el.removeEventListener('keydown', rec.keyDownHandler, {capture:true});
...
Pfad: ${path || knownHosts.path || 'known_hosts'}`);
  ...
  function openLocalVaultSettings() { setSettingsPanel('securitySync'); setView('settings'); setShowLocalVaultPrompt(false); setTimeout(() => localVaultSettingsRef.current?.scrollIntoView({ behavior:'smooth', block:'start' }), 60); }
  function openLocalVaultUnlock() { setSidebarCollapsed(false); closeHostEditor(); setSettingsPanel('securitySync'); setView('settings'); setShowLocalVaultPrompt(true); setTimeout(() => localVaultSettingsRef.current?.scrollIntoView({ behavior:'smooth', block:'start' }), 60); }
  ...
  function openUpdateSettings() { setSettingsPanel('updates'); setWorkspaceView('settings'); closeHostEditor(); setSidebarCollapsed(false); setTimeout(() => updateSettingsRef.current?.scrollIntoView({ behavior:'smooth', block:'start' }), 60); }
  function openSyncSettings() { setSettingsPanel('securitySync'); setWorkspaceView('settings'); closeHostEditor(); setSidebarCollapsed(false); setShowLocalVaultPrompt(false); setTimeout(() => (syncActionsRef.current || syncSettingsRef.current)?.scrollIntoView({ behavior:'smooth', block:'center' }), 60); }
